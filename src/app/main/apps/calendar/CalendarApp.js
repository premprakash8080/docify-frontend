...
function CalendarApp(props) {
  ...
  useEffect(() => {
    // Fetch initial events for current date and month view
    const today = format(new Date(), 'yyyy-MM-dd');
    dispatch(getEvents({ date: today, view: 'month' }));
    dispatch(getLabels());
  }, [dispatch]);
  ...
  const handleDates = (rangeInfo) => {
    ...
    const { view, start } = rangeInfo;
    const viewType = view.type; // 'dayGridMonth', 'timeGridWeek', 'timeGridDay'
    let apiView = 'month';
    ...
    dispatch(getEvents({ date: dateString, view: apiView }));
  };
