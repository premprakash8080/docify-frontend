import {type ManagementType, permissionManagementData} from '@/views/apps/users/permissions/data'
            ...
            cell: ({row}) => (
                <div className="d-flex gap-1 flex-wrap">
                    {row.original.roles.map((role, idx) => (
                        <span key={idx}
                              className={`badge bg-${role.variant}-subtle text-${role.variant} badge-label fs-xxs fw-semibold`}>
              {role.label}
            </span>
                    ))}
                </div>
