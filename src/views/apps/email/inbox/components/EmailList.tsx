import { actions, emails } from '@/views/apps/email/data'
import type { EmailType } from '@/views/apps/email/types'
...
const EmailList = ({ toggleSidebar }: { toggleSidebar: () => void }) => {
  ...
  return (
    <Card className="h-100 mb-0 rounded-start-0 flex-grow-1 border-start-0">
      ...
      <SimpleBar className="card-body p-0" style={{ height: 'calc(100% - 100px)' }} data-simplebar data-simplebar-md>
                  ...
                  <tr className={`position-relative ${email.isRead ? 'mark-as-read' : ''}`} key={email.id}>
                    ...
                    <td>
                      <div className="d-flex align-items-center gap-2">
                        {email.avatar ? (
                          <img src={email.avatar} alt="user avatar" className="avatar-xs rounded-circle" />
                        ) : (
                    ...
                    <td>
                      <Link to={`/inbox/${email.id}`} className="link-reset fs-base fw-medium stretched-link">
                        {email.subject}
                      </Link>
