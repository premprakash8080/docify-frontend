...
import {getReviewChartOptions, productReviews, type ProductReviewType, reviews} from '@/views/ecommerce/reviews/data'
...
const ProductReviews = () => {
    const columns: ColumnDef<ProductReviewType, any>[] = [
        ...
        columnHelper.accessor('productName', {
            ...
            cell: ({row}) => (
                <div className="d-flex align-items-center">
                    ...
                    <div>
                        <h5 className="mb-0">
                            <Link to="/ecommerce/products/1" className="link-reset">
            ...
            cell: ({row}) => (
                <div className="d-flex justify-content-start align-items-center gap-2">
                    <div className="avatar avatar-sm">
                        <img src={row.original.avatar} alt="avatar-8" className="img-fluid rounded-circle"/>
                    </div>
                    <div>
        ...
        columnHelper.accessor('status', {
            ...
            cell: ({row}) => (
                <span
                    className={`badge ${row.original.status === 'pending' ? 'badge-soft-warning' : 'badge-soft-success'} fs-xxs`}>
                ...
                <Row className="g-0 align-items-center">
                            ...
                            <Col xl={7}>
                                <div className="d-flex align-items-center gap-4 p-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="pe-2 h6 text-muted mb-2 lh-base">Feedback collected from real
                            ...
                            <Col xl={5}>
                                <div className="p-3">
                                    {reviews.map((review, idx) => {
                                        return (
                                            <div key={idx}
                                                 className={clsx('d-flex align-items-center gap-2', {'mb-2': idx != reviews.length - 1})}>
                                                <div className="flex-shrink-0" style={{width: 50}}>
                                                    {reviews.length - idx} Star
                                                </div>
                                                <ProgressBar now={review.progress} variant="primary" className="w-100"
                                                             style={{height: 8}}/>
                    ...
                    <Col xl={6}>
                        <div className="pe-3 ps-1">
                            <CustomApexChart getOptions={getReviewChartOptions} series={getReviewChartOptions().series}
                                             type="area" height={185}/>
                        </div>
                    </Col>
                </Row>
            </CardHeader>
                ...
                <div className="d-flex align-items-center gap-2">
                    ...
                    <Dropdown>
                        <DropdownToggle className="btn btn-default  drop-arrow-none" type="button">
                            {statusFilterValue === 'All' ? 'View All' : statusFilterValue} <TbChevronDown
                            className="align-middle ms-1"/>
                        </DropdownToggle>
                        <DropdownMenu className="dropdown-menu-end">
                            {['All', 'Pending', 'Published', 'disabled'].map((option) => (
                                <Dropdown.Item as="button" key={option} active={statusFilterValue === option}
