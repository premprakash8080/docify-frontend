...
import {Direction, type IRenderThumbParams, type IRenderTrackParams} from 'react-range/lib/types'

import Rating from '@/components/Rating.tsx'
import {getColor} from '@/helpers/color.ts'
...
const ProductFilter = ({isOffcanvasOpen, setIsOffcanvasOpen}: {
    ...
    return (
        <Col xl={3}>
            <Offcanvas responsive="lg" placement="start" show={isOffcanvasOpen}
                            ...
                            <Nav className="flex-column">
                                {categories.map((category) => (
                                    <NavItem key={category.id}
                                             className="d-flex text-muted justify-content-between align-items-center py-1">
                                        <FormCheck type="checkbox" id={`cat-${category.id}`} label={category.name}
                        ...
                        <div className="p-3 border-bottom border-dashed">
                            ...
                            {brands.map((brand) => (
                                <NavItem key={brand.id}
                                         className="d-flex justify-content-between text-muted align-items-center py-1">
                                    <FormCheck type="checkbox" id={`brand-${brand.id}`} label={brand.name}
